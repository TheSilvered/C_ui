...
struct UIContext {
    ...
    UIWindow window;
    UIElement *root;
    UI__ElementBucket *_freeElement;
    size_t _freeElementCount;
...
bool UI__AppendChild(UI__Children *children, UIElement *child) {
...
bool UI__Element_AddChild(UIElement *parent, UIElement *child) {
    ...
    return UI__AppendChild(&parent->children, child);
}
...
bool UIContext_Init(UIContext *ctx, size_t maxElementCount, void *userData) {
    ...
    ctx->_freeElement = NULL;
    ctx->maxFreeElementCount = maxElementCount;
    ctx->_freeElementCount = 0;
...
UIElement *UI__Context_AllocElement(UIContext *ctx) {
    ...
    if (ctx->_freeElement != NULL) {
        element = &ctx->_freeElement->element;
        ctx->_freeElement = ctx->_freeElement->next;
        ctx->_freeElementCount--;
    } else {
...
void UI__Context_FreeElement(UIContext *ctx, UIElement *element) {
    if (ctx->_freeElementCount < ctx->maxFreeElementCount) {
        UI__ElementBucket *bucket = (UI__ElementBucket *)element;
        bucket->next = ctx->_freeElement;
        ctx->_freeElement = bucket;
        ctx->_freeElementCount++;
    } else {
        UI_MemFree(element);
    }
...
bool UI__ElementFillWidth(UIElement *element) {
    ...
    for (size_t i = 0, n = element->children.len; i < n; i++) {
        ...
        if (!UI__AppendChild(fillChildren, child))
            return false;
